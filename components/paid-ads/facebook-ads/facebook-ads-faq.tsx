...
import { facebookAdsFaqs } from "./facebook-ads-faq-data"

function stripHtml(html: string) {
  return html.replace(/<[^>]+>/g, "")
}

export function FacebookAdsFaq() {
  const [openIdx, setOpenIdx] = useState<number | null>(0)

  return (
    <section
      id="facebook-ads-faq"
      aria-labelledby="facebook-ads-faq-heading"
      className="relative"
    >
      ...
      <div className="section-container section-y relative z-10">
        <SectionHeader
          pill="Questions"
          heading={
            <span id="facebook-ads-faq-heading">
              Common Facebook Ads questions,{" "}
              <span className="whitespace-nowrap">
                <span className="accent-underline">straight answers</span>.
              </span>
            </span>
          }
          paragraph="Everything service businesses ask before hiring us for Facebook Ads management &mdash; pricing, minimum spend, account ownership, iOS 14.5 tracking, contract terms, and what happens if you ever leave."
        />
        ...
        >
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
            <span className="text-[15.5px]" style={{ color: "var(--muted)" }}>
              Still have questions about Facebook Ads management?
            </span>
            <SecondaryButton asLink href="#contact">
              Talk to a Facebook Ads strategist
            </SecondaryButton>
          </div>
        </div>
      </div>
...
export function FacebookAdsFaqJsonLd() {
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: facebookAdsFaqs.map((f) => ({
      "@type": "Question",
      name: stripHtml(f.q),
