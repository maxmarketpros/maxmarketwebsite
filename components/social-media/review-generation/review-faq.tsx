...
import {
  reviewFaqs,
  reviewFaqCategories,
  type ReviewFaqCategory,
} from "./review-faq-data"

function stripHtml(html: string) {
  return html.replace(/<[^>]+>/g, "")
}

const categoryColors: Record<ReviewFaqCategory, string> = {
  Compliance: "#22C55E",
  Speed: "#F59E0B",
  Platforms: "#1677FF",
  Setup: "#0EA5E9",
...
export function ReviewFaq() {
  const [activeCat, setActiveCat] = useState<"All" | ReviewFaqCategory>("All")
  const [openIdx, setOpenIdx] = useState<number | null>(0)

  const visible = reviewFaqs.filter(
  ...
  return (
    <section
      id="review-faq"
      aria-labelledby="review-faq-heading"
      className="relative"
    >
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="absolute top-[5%] left-[-5%] w-[420px] h-[420px] rounded-full"
          style={{
            background:
      ...
      <div className="section-container section-y relative z-10">
        <SectionHeader
          pill="Review Generation FAQ"
          heading={
            <span id="review-faq-heading">
              The questions service businesses ask{" "}
              <span className="whitespace-nowrap">
                <span className="accent-underline">before signing on</span>.
              </span>
            </span>
          }
          paragraph="Everything you should know about review generation, automation, compliance, and the difference between us and a generic review-software subscription."
        />
        ...
        >
          ...
          >
            {reviewFaqCategories.map((cat) => {
              const isActive = activeCat === cat
              const cat_color =
                cat === "All"
                  ? "var(--accent)"
                  : categoryColors[cat as ReviewFaqCategory]
              return (
                <button
                  key={cat}
                  type="button"
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
            <span className="text-[15.5px]" style={{ color: "var(--muted)" }}>
              Got an industry-specific question?
            </span>
            <SecondaryButton asLink href="#contact">
              Talk to a reputation strategist
            </SecondaryButton>
          </div>
        </div>
      </div>
...
export function ReviewFaqJsonLd() {
  const faqSchema = {
