...
import {
  videoEditingFaqs,
  videoEditingFaqCategories,
  type VideoEditingFaqCategory,
} from "./video-editing-faq-data"

function stripHtml(html: string) {
  return html.replace(/<[^>]+>/g, "")
}

const categoryColors: Record<VideoEditingFaqCategory, string> = {
  Turnaround: "#1677FF",
  "Files & Formats": "#0EA5E9",
  Revisions: "#22C55E",
  Pricing: "#F59E0B",
  Quality: "#EC4899",
}

export function VideoEditingFaq() {
  const [activeCat, setActiveCat] = useState<"All" | VideoEditingFaqCategory>(
    "All",
  )
  const [openIdx, setOpenIdx] = useState<number | null>(0)

  const visible = videoEditingFaqs.filter(
  ...
  return (
    ...
    >
      ...
      <div className="section-container section-y relative z-10">
        <SectionHeader
          pill="Video Editing FAQ"
          heading={
            <span id="video-editing-faq-heading">
              Questions before you{" "}
              <span className="whitespace-nowrap">
                <span className="accent-underline">hand off the edit</span>?
              </span>
            </span>
          }
          paragraph="The most common questions service businesses ask about monthly video editing — turnaround, file formats, revisions, pricing, and how we deliver consistent quality across every short-form platform."
        />
        ...
        >
          ...
          >
            {videoEditingFaqCategories.map((cat) => {
              const isActive = activeCat === cat
              const cat_color =
                cat === "All"
                  ? "var(--accent)"
                  : categoryColors[cat as VideoEditingFaqCategory]
              return (
                <button
                  key={cat}
                  type="button"
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
            <span className="text-[15.5px]" style={{ color: "var(--muted)" }}>
              Don&rsquo;t see your video editing question?
            </span>
...
export function VideoEditingFaqJsonLd() {
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: videoEditingFaqs.map((f) => ({
      "@type": "Question",
      name: f.q,
