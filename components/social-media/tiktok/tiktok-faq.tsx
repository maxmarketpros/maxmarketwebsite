...
import {
  tiktokFaqs,
  tiktokFaqCategories,
  type TiktokFaqCategory,
} from "./tiktok-faq-data"

function stripHtml(html: string) {
  return html.replace(/<[^>]+>/g, "")
}

const categoryColors: Record<TiktokFaqCategory, string> = {
  Onboarding: "#22D3EE",
  Content: "#EC4899",
  Strategy: "#22C55E",
  Pricing: "#F59E0B",
...
export function TiktokFaq() {
  const [activeCat, setActiveCat] = useState<"All" | TiktokFaqCategory>("All")
  const [openIdx, setOpenIdx] = useState<number | null>(0)

  const visible = tiktokFaqs.filter(
  ...
  return (
    <section
      id="tiktok-faq"
      aria-labelledby="tiktok-faq-heading"
      className="relative"
    >
      ...
      <div className="section-container section-y relative z-10">
        <SectionHeader
          pill="TikTok marketing services FAQ"
          heading={
            <span id="tiktok-faq-heading">
              <span className="accent-underline">FAQs</span> about our TikTok
              marketing services.
            </span>
          }
          paragraph="The most common questions service businesses ask before signing on for done-for-you TikTok marketing services — onboarding, content control, FYP strategy, pricing, and how we report results."
        />
        ...
        >
          ...
          >
            {tiktokFaqCategories.map((cat) => {
              const isActive = activeCat === cat
              const cat_color =
                cat === "All"
                  ? "var(--accent)"
                  : categoryColors[cat as TiktokFaqCategory]
              return (
                <button
                  key={cat}
                  type="button"
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
            <span className="text-[15.5px]" style={{ color: "var(--muted)" }}>
              Still curious about TikTok marketing services?
            </span>
...
export function TiktokFaqJsonLd() {
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: tiktokFaqs.map((f) => ({
      "@type": "Question",
      name: f.q,
