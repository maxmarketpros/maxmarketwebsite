...
import { pressReleaseFaqs } from "./press-releases-faq-data"

function stripHtml(html: string) {
  return html.replace(/<[^>]+>/g, "")
}

export function PressReleasesFaq() {
  const [openIdx, setOpenIdx] = useState<number | null>(0)

  return (
    <section
      id="press-releases-faq"
      aria-labelledby="press-releases-faq-heading"
      className="relative"
    >
      ...
      <div className="section-container section-y relative z-10">
        <SectionHeader
          pill="Press Release FAQ"
          heading={
            <span id="press-releases-faq-heading">
              Buying answers, before you{" "}
              <span className="whitespace-nowrap">
                <span className="accent-underline">commit a budget</span>.
              </span>
            </span>
          }
          paragraph="The most common questions we get from businesses researching where to buy press releases — pricing, outlets, turnaround, guarantees, and what separates real placements from syndicated junk."
        />
...
export function PressReleasesFaqJsonLd() {
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: pressReleaseFaqs.map((f) => ({
      "@type": "Question",
      name: f.q,
