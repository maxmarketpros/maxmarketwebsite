...
import { paymentsFaqs } from "./payments-faq-data"

export function PaymentsFaq() {
  const [openIdx, setOpenIdx] = useState<number | null>(0)

  return (
    <section aria-labelledby="payments-faq-heading" className="relative">
      <div className="section-container section-y">
        <SectionHeader
          pill="FAQ"
          heading={
            <span id="payments-faq-heading">
              Payments & invoicing,{" "}
              <span className="accent-underline">
                explained <span className="whitespace-nowrap">plainly.</span>
              </span>
            </span>
          }
          paragraph="The real questions service-business owners ask about payment processing, deposits, and recurring billing."
        />

        <div className="mt-12 max-w-[820px] mx-auto" style={{ animation: "fadeInUp 0.7s ease-out both" }}>
          <ul className="space-y-3">
            {paymentsFaqs.map((faq, idx) => {
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
            ...
            <SecondaryButton asLink href="#contact">Talk to a payment specialist</SecondaryButton>
          </div>
        </div>
      </div>
    </section>
