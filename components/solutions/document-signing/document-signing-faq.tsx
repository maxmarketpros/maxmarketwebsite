...
import { docSigningFaqs } from "./document-signing-faq-data"

export function DocumentSigningFaq() {
  const [openIdx, setOpenIdx] = useState<number | null>(0)

  return (
    <section aria-labelledby="doc-signing-faq-heading" className="relative">
      <div className="section-container section-y">
        <SectionHeader
          pill="FAQ"
          heading={
            <span id="doc-signing-faq-heading">
              E-signatures,{" "}
              <span className="accent-underline">
                without the <span className="whitespace-nowrap">legalese.</span>
              </span>
            </span>
          }
          paragraph="The real questions we hear from contractors and service-business owners switching from paper contracts."
        />

        <div className="mt-12 max-w-[820px] mx-auto" style={{ animation: "fadeInUp 0.7s ease-out both" }}>
          <ul className="space-y-3">
            {docSigningFaqs.map((faq, idx) => {
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
            <span className="text-[15.5px]" style={{ color: "var(--muted)" }}>Want to see a sample document?</span>
            <SecondaryButton asLink href="#contact">Request a sample</SecondaryButton>
          </div>
        </div>
      </div>
    </section>
