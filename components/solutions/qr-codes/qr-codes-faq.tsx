...
import { qrCodesFaqs } from "./qr-codes-faq-data"

export function QRCodesFaq() {
  const [openIdx, setOpenIdx] = useState<number | null>(0)

  return (
    <section aria-labelledby="qr-codes-faq-heading" className="relative">
      <div className="section-container section-y">
        <SectionHeader
          pill="FAQ"
          heading={
            <span id="qr-codes-faq-heading">
              QR codes,{" "}
              <span className="accent-underline">
                <span className="whitespace-nowrap">demystified.</span>
              </span>
            </span>
          }
          paragraph="Quick answers for service businesses thinking about adding QR codes to their offline marketing."
        />

        <div className="mt-12 max-w-[820px] mx-auto" style={{ animation: "fadeInUp 0.7s ease-out both" }}>
          <ul className="space-y-3">
            {qrCodesFaqs.map((faq, idx) => {
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
            <span className="text-[15.5px]" style={{ color: "var(--muted)" }}>Want a sample branded QR code?</span>
