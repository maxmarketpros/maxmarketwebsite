...
import { SERVICEPRO_ACCENT, SERVICEPRO_ACCENT_DARK } from "@/lib/servicepro-data"
import { serviceproFaqs } from "./servicepro-faq-data"

function stripHtml(html: string) {
  return html.replace(/<[^>]+>/g, "")
}

export function ServiceProFaq() {
  const [openIdx, setOpenIdx] = useState<number | null>(0)

  return (
    <section
      id="servicepro-faq"
      aria-labelledby="servicepro-faq-heading"
      className="relative"
    >
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="absolute top-[5%] right-[-5%] w-[420px] h-[420px] rounded-full"
          style={{ background: `radial-gradient(circle, ${SERVICEPRO_ACCENT}1F 0%, transparent 70%)` }}
        />
      </div>
      ...
      <div className="section-container section-y relative z-10">
        <SectionHeader
          pill="ServicePro FAQ"
          heading={
            <span id="servicepro-faq-heading">
              Questions about{" "}
              <span className="whitespace-nowrap">
                <span className="accent-underline">ServicePro</span>?
              </span>
            </span>
          }
          paragraph="The questions we hear most often before picking ServicePro — what’s in it, what isn’t, how the AI works, and how fast results show up."
        />
                ...
                <li key={faq.q} style={{ borderColor: "var(--border-color)" }}>
                  ...
                  >
                    <span
                      ...
                      style={{
                        background: isOpen
                          ? `linear-gradient(180deg, #FBBF24 0%, ${SERVICEPRO_ACCENT} 60%, ${SERVICEPRO_ACCENT_DARK} 100%)`
                          : `${SERVICEPRO_ACCENT}14`,
                        color: isOpen ? "#fff" : SERVICEPRO_ACCENT_DARK,
...
export function ServiceProFaqJsonLd() {
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: serviceproFaqs.map((f) => ({
      "@type": "Question",
      name: f.q,
