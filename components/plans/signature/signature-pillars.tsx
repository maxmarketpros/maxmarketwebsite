...
function FeatureCard({ feature, pillarAccent }: { feature: SignatureFeature; pillarAccent: string }) {
...
function Pillar({ pillar }: { pillar: SignaturePillar }) {
  const Icon = getIcon(pillar.iconName)
  const anchor = PILLAR_ANCHORS[pillar.slug] ?? pillar.slug

  return (
    <div id={`pillar-${anchor}`} className="relative scroll-mt-20">
      {/* Pillar header */}
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 sm:gap-2.5">
              ...
              <span
                ...
                style={{ background: `${pillar.accent}14`, color: pillar.accent }}
              >
                Pillar {pillar.number} · {pillar.features.length} {pillar.features.length === 1 ? "feature" : "features"}
              </span>
            </div>
            <h3
...
export function SignaturePillars() {
  return (
    <section
      ...
      aria-labelledby="signature-pillars-heading"
    ...
    >
      ...
      <div className="section-container section-y relative z-10">
        <SectionHeader
          pill="The five pillars"
          heading={
            <span id="signature-pillars-heading">
              26 features, organized into{" "}
              <span className="whitespace-nowrap">
                5 working <span className="accent-underline">pillars</span>.
              </span>
            </span>
          }
          paragraph="Every Signature feature does a real job — and every job belongs to one of five pillars: getting found, producing the content, running the channels, converting the leads, and proving the ROI."
        />

        <div className="mt-16 space-y-20 lg:space-y-24">
          {signaturePillars.map((p) => (
